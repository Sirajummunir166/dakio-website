...
/**
 * True when the router owns this href, i.e. it is one of our own pages and can
 * be a client-side <Link>. False for the app subdomain, the verbatim
 * /prototypes embeds (real files, and excluded from the middleware matcher),
 * mailto:/tel:, and bare `#hash` jumps — all of which must stay plain anchors.
 * Same rule as href() uses to decide what to leave alone.
 */
export const isRouterHref = path => typeof path === "string" && path.length > 0 && !isExternalish(path);
