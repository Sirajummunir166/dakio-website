...
/**
 * `offers` overrides the committed offer copy with the live catalogue. The
 * fallback is deliberate and it is the same tradeoff as the pricing page: stale
 * structured data beats emitting none. What it must NEVER do again is advertise
 * a plan that has been withdrawn from sale, which is why the caller passes the
 * live list whenever it has one.
 */
export const softwareApplication = (lang = "en", offers = null) => ({
  "@context": "https://schema.org",
  ...
  offers: (offers?.length ? offers : pack(lang).offers).map(o => ({ "@type": "Offer", ...o })),
});
...
export function jsonLdFor(route, lang = "en", { offers = null } = {}) {
  const blocks = [breadcrumbs(route, lang)];
  if (route === "/") blocks.push(organization(lang), softwareApplication(lang, offers));
