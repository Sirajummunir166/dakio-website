...
export default async function Home({ params }) {
  ...
  return (
    <div style={{ fontFamily: "var(--dk-font-sans), var(--dk-font-bn), sans-serif", color: "#1A1D12", background: "#F4F2EA", overflowX: "hidden" }}>
      ...
      {/* ================= TRUST — Nova's office as a living field ================= */}
      {/* One argument, one instrument: the claim, then the office it is made in —
          every ring a department and its grade, every line Nova at work, amber
          waiting for the founder's call. */}
      <div id="trust" className="m-org-section" style={{ maxWidth: 1200, margin: "0 auto", padding: "96px 28px 20px", textAlign: "center" }}>
        <div data-reveal>
          ...
          <p style={{ margin: "18px auto 0", maxWidth: 560, fontSize: 16, lineHeight: 1.6, color: "#4C4F42", ...T.lead }}>{c.trust.lede}</p>
        </div>
        <div data-reveal style={{ marginTop: 34 }}>
          <NovaField
            copy={c.trust.field}
            badge={MONO.trustFieldBadge}
            storeName="Shahrqee"
