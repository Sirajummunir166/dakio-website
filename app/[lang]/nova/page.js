...
import novaEn, { MONO } from "../../../content/copy/nova.en";
import novaBn from "../../../content/copy/nova.bn";

const COPY = { en: novaEn, bn: novaBn };
const ROUTE = "/nova";
...
const agentTag = (m, i) => ({
  fontFamily: MONOFONT, fontSize: 7.5, fontWeight: 600, letterSpacing: "0.08em", padding: "3px 8px", borderRadius: 99,
  ...(i === 0
...
export default async function NovaPage({ params }) {
  const { lang } = await params;
  const c = COPY[lang] || COPY.en;
  const T = type(lang);
  ...
  return (
    <div style={{ fontFamily: "var(--dk-font-sans), var(--dk-font-bn), sans-serif", color: "#1A1D12", background: "#F4F2EA", overflowX: "hidden" }}>
      ...
      <Nav lang={lang} route={ROUTE} active="nova" ctaHref="#cta" style={{ position: "sticky", top: 0, zIndex: 60 }} />
      ...
      <div className="m-bleed-wrap m-hero-wrap" style={{ maxWidth: 1200, margin: "18px auto 0", padding: "0 20px" }}>
        <div className="m-pad-hero m-bleed" style={{ position: "relative", borderRadius: 36, background: "#0F120B", color: "#E9EFDC", overflow: "hidden", padding: "88px 64px" }}>
          ...
          <div className="m-grid" style={{ position: "relative", display: "grid", gridTemplateColumns: "1.1fr 0.9fr", gap: 40, alignItems: "center" }}>
            <div>
              <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "7px 14px", borderRadius: 99, border: "1px solid rgba(198,242,62,0.35)", fontFamily: MONOFONT, fontSize: 10, fontWeight: 600, letterSpacing: "0.12em", color: "#C6F035", animation: "heroUp .6s ease both" }}>{MONO.heroBadge}</div>
              <h1 className="m-h1" style={{ margin: "22px 0 0", fontSize: 66, lineHeight: 1.02, letterSpacing: "-2.6px", fontWeight: 800, color: "#FBFBF4", animation: "heroUp .6s .08s ease both", ...T.h1 }}>{c.hero.h1}</h1>
              <p style={{ margin: "22px 0 0", fontSize: 17, lineHeight: 1.6, color: "#A9AD98", maxWidth: 430, animation: "heroUp .6s .16s ease both", ...T.lead }}>{c.hero.sub}</p>
              <div className="m-wrap" style={{ display: "flex", gap: 12, marginTop: 30, animation: "heroUp .6s .24s ease both" }}>
                <a href="#cta" className="hv-up2-glow" style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "15px 26px", borderRadius: 99, background: "#C6F035", color: "#0F120B", fontSize: 15, fontWeight: 700, ...T.label }}>{c.hero.ctaPrimary}</a>
                <a href="/prototypes/Nova HQ Prototype v7.dc.html" className="hv-bg-wash07" style={{ display: "inline-flex", alignItems: "center", padding: "15px 24px", borderRadius: 99, border: "1px solid rgba(233,239,220,0.25)", color: "#E9EFDC", fontSize: 15, fontWeight: 700, ...T.label }}>{c.hero.ctaSecondary}</a>
              </div>
              <div style={{ marginTop: 28, display: "inline-flex", alignItems: "center", gap: 10, padding: "8px 15px", borderRadius: 99, border: "1px solid rgba(198,242,62,0.2)", fontFamily: MONOFONT, fontSize: 9, letterSpacing: "0.1em", color: "#878B76", animation: "heroUp .6s .32s ease both" }}>{MONO.heroRole}</div>
            </div>
            <div className="m-orbcol" style={{ position: "relative", height: 400 }}>
              ...
              <div className="m-nowrap-off" style={{ position: "absolute", left: "50%", bottom: 0, transform: "translateX(-50%)", fontFamily: MONOFONT, fontSize: 9, letterSpacing: "0.14em", color: "#878B76", whiteSpace: "nowrap", textAlign: "center", width: "100%" }}>{MONO.heroOrbit}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "96px 28px 20px" }}>
        <div style={{ textAlign: "center", marginBottom: 40 }} data-reveal>
          <div style={kicker}>{MONO.dayKicker}</div>
          <h2 className="m-h2" style={{ margin: "14px auto 0", fontSize: 52, lineHeight: 1.05, letterSpacing: "-2px", fontWeight: 800, ...T.h2 }}>{c.day.h2}</h2>
        </div>
          ...
          {c.day.items.map(dy => (
            <div key={dy.t} style={{ position: "relative" }}>
              ...
              <div style={{ marginTop: 14, padding: 18, borderRadius: 18, background: "#FBFAF5", border: "1px solid rgba(26,29,18,0.07)", minHeight: 130 }}>
                <div style={{ fontSize: 14.5, fontWeight: 800, letterSpacing: "-0.2px", ...T.h3 }}>{dy.n}</div>
                <div style={{ fontSize: 12, color: "#6B6D60", marginTop: 6, lineHeight: 1.55, ...T.small }}>{dy.d}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="m-bleed-wrap" style={{ maxWidth: 1200, margin: "0 auto", padding: "96px 28px 20px" }}>
        <div data-reveal className="m-pad-band m-bleed" style={{ borderRadius: 32, background: "#1A1D12", color: "#F0EFE6", padding: "52px 56px", textAlign: "center" }}>
          <div style={{ fontFamily: MONOFONT, fontSize: 10, fontWeight: 600, letterSpacing: "0.14em", color: "#8CBF33" }}>{MONO.ruleKicker}</div>
          <h2 className="m-h2b" style={{ margin: "14px auto 0", fontSize: 40, lineHeight: 1.1, letterSpacing: "-1.4px", fontWeight: 800, color: "#FBFBF4", maxWidth: 640, ...T.h2b }}>{c.rule.h2}</h2>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, marginTop: 32, flexWrap: "wrap" }}>
            {c.rule.pipeline.map((step, i) => (
              <div key={MONO.pipeline[i]} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{ padding: "13px 20px", borderRadius: 14, background: "rgba(255,255,255,0.06)", border: "1px solid rgba(198,242,62,0.25)" }}>
                  <div style={{ fontFamily: MONOFONT, fontSize: 8.5, letterSpacing: "0.1em", color: "#8CBF33" }}>{MONO.pipeline[i]}</div>
                  <div style={{ fontSize: 13.5, fontWeight: 700, marginTop: 3, color: "#FBFBF4", ...T.chip }}>{step}</div>
                </div>
                {i < c.rule.pipeline.length - 1 ? <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#C6F035" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14M13 6l6 6-6 6" /></svg> : null}
              </div>
            ))}
          </div>
          <div style={{ marginTop: 24, fontSize: 13, color: "#A9AD98", ...T.small }}>{c.rule.note}</div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "96px 28px 20px" }}>
        <div className="m-grid m-gap" style={{ display: "grid", gridTemplateColumns: "0.9fr 1.1fr", gap: 56, alignItems: "center" }} data-reveal>
          <div>
            <div style={kicker}>{MONO.deskKicker}</div>
            <h2 className="m-h2" style={{ margin: "14px 0 0", fontSize: 52, lineHeight: 1.05, letterSpacing: "-2px", fontWeight: 800, ...T.h2 }}>
              {c.desk.h2}
            </h2>
            <p style={{ margin: "18px 0 0", fontSize: 15.5, lineHeight: 1.65, color: "#6B6D60", maxWidth: 380, ...T.body }}>{c.desk.p}</p>
            <div style={{ marginTop: 22 }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontFamily: MONOFONT, fontSize: 9, letterSpacing: "0.1em", color: "#6B6D60" }}><span>{MONO.deskTrust}</span><span>{MONO.deskTrustPct}</span></div>
              ...
              <div style={{ marginTop: 8, fontSize: 11.5, color: "#6B6D60", ...T.small }}>{c.desk.trustNote}</div>
            </div>
          </div>
          <div style={{ borderRadius: 28, background: "#0F120B", padding: 26, color: "#E9EFDC", boxShadow: "0 34px 80px rgba(15,18,11,0.35)" }}>
            <div style={{ fontFamily: MONOFONT, fontSize: 9, letterSpacing: "0.14em", color: "#878B76" }}>{MONO.deskWaiting}</div>
            <DecisionCard lang={lang} copy={c.desk.card} mono={MONO} />
            <div style={{ marginTop: 12, padding: "14px 16px", borderRadius: 14, background: "rgba(198,242,62,0.07)", border: "1px dashed rgba(198,242,62,0.3)" }}>
              <div style={{ fontFamily: MONOFONT, fontSize: 8.5, letterSpacing: "0.12em", color: "#C6F035" }}>{MONO.deskPromotion}</div>
              <div style={{ fontSize: 12, color: "#A9AD98", marginTop: 5, lineHeight: 1.55, ...T.small }}>{c.desk.promotion}</div>
            </div>
          </div>
        </div>
      </div>
          ...
          <div style={{ borderRadius: 28, background: "#0F120B", padding: 28, color: "#E9EFDC" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 15.5, fontWeight: 800, color: "#FBFBF4", ...T.h3 }}>{c.voice.calling}</div>
                <div style={{ fontSize: 12, color: "#A9AD98", marginTop: 2, ...T.small }}>{c.voice.callingSub}</div>
              </div>
            </div>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 18 }}>
              <span style={{ flex: 1, textAlign: "center", padding: "11px 0", borderRadius: 99, background: "#C6F035", color: "#0F120B", fontSize: 13, fontWeight: 700, cursor: "pointer", ...T.label }}>{c.voice.answer}</span>
              <span style={{ flex: 1, textAlign: "center", padding: "11px 0", borderRadius: 99, border: "1px solid rgba(233,239,220,0.2)", color: "#A9AD98", fontSize: 13, fontWeight: 600, cursor: "pointer", ...T.label }}>{c.voice.sendToDesk}</span>
            </div>
          </div>
          <div>
            <div style={kicker}>{MONO.voiceKicker}</div>
            <h2 className="m-h2" style={{ margin: "14px 0 0", fontSize: 52, lineHeight: 1.05, letterSpacing: "-2px", fontWeight: 800, ...T.h2 }}>{c.voice.h2}</h2>
            <p style={{ margin: "18px 0 0", fontSize: 15.5, lineHeight: 1.65, color: "#6B6D60", maxWidth: 380, ...T.body }}>{c.voice.p}</p>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 22 }}>
              {c.voice.chips.map(ch => (
                <span key={ch} style={{ padding: "8px 15px", borderRadius: 99, background: "#FBFAF5", border: "1px solid rgba(26,29,18,0.1)", fontSize: 12, fontWeight: 700, ...T.chip }}>{ch}</span>
              ))}
              <span style={{ padding: "8px 15px", borderRadius: 99, background: "#1A1D12", color: "#C6F035", fontSize: 12, fontWeight: 700 }}>{c.voice.chipLangs}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "96px 28px 20px", textAlign: "center" }}>
        <div data-reveal>
          <div style={kicker}>{MONO.agentsKicker}</div>
          <h2 className="m-h2" style={{ margin: "14px auto 0", fontSize: 52, lineHeight: 1.05, letterSpacing: "-2px", fontWeight: 800, maxWidth: 680, ...T.h2 }}>{c.agents.h2}</h2>
          <p style={{ margin: "16px auto 0", fontSize: 15, color: "#6B6D60", maxWidth: 460, lineHeight: 1.6, ...T.body }}>{c.agents.p}</p>
        </div>
        ...
        <div data-reveal className="m-wrap" style={{ marginTop: 26, display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 10, padding: "12px 22px", borderRadius: 99, background: "#1A1D12", color: "#F0EFE6", fontSize: 13, fontWeight: 600, ...T.label }}>
          {c.agents.founderOnlyLabel}&nbsp;<span style={{ fontFamily: MONOFONT, fontSize: 10, letterSpacing: "0.08em", color: "#C6F035" }}>{MONO.founderOnly}</span>
        </div>
      </div>
      ...
      <div id="cta" className="m-bleed-wrap" style={{ maxWidth: 1200, margin: "80px auto 0", padding: "0 20px 60px" }}>
        <div data-reveal className="m-pad-cta m-bleed" style={{ borderRadius: 36, background: "#C6F035", padding: "76px 40px", textAlign: "center", position: "relative", overflow: "hidden" }}>
          ...
          <h2 className="m-cta-h2" style={{ position: "relative", margin: "0 auto", fontSize: 58, lineHeight: 1.03, letterSpacing: "-2.4px", fontWeight: 800, maxWidth: 700, ...T.ctaH2 }}>{c.cta.h2}</h2>
